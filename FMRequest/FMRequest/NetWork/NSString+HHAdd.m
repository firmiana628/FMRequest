...
-(NSString *)cutoff:(NSInteger)limit
{
    if (self.length > limit) {
        return [self substringWithRange:NSMakeRange(0, limit)];
    }
    return self;
}

-(NSString *)cutoffWithEllipsis:(NSInteger)limit
{
    if (self.length > limit) {
        return [[self substringWithRange:NSMakeRange(0, limit)] stringByAppendingString:@"..."];
    }
    return self;
}
